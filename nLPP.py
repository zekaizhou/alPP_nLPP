...
def Update_P(data, D, O_W, n_dims):  #更新特征矩阵P
    ...
    return W_n, data_ndim, eig_vec_picked
...
def nlpp(data, lam, n_dims, n_neighbors, maxN, beta):
    tol = 1e-10
    n = 1
    ...
    while True:  #交替更新,直到收敛
        W_n, data_ndim, eig_vec = Update_P(data, D, O_W, n_dims)
        Wi = (W_n, )
        W_t = W_t + Wi
        if (np.max(np.abs(W_t[n] - W_t[n - 1])) < tol):
            break
        n = n + 1
        if n > maxN:
            break
        O_W = Update_Omega(D, data_ndim, n, W_t)
...
if __name__ == "__main__":
    ...
    data_2d, w = nlpp(X, lam=0.01, n_dims=2, n_neighbors=5, maxN=20, beta=1.9)
